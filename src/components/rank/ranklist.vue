    ...
    <div>
      <div class="topheader">
        ...
        <div class="button" @click="$router.go(-1)">
          <p class="iconfont icon-arrow-left-bold">排行榜</p>
        </div>
      </div>
    </div>
    <dl class="musiclist">
      ...
      <dd v-for="(item, index) in playlist.tracks" :key="index" @click="playmusic(item.id,item.name,item.ar)">
        ...
        <!-- v-show="index < 3  数据不大于3个 -->
        <div class="bottom" v-for="(key, index) in item.ar" :key="index" v-show="index < 3">{{key.name}}&nbsp;&nbsp;</div>
      </dd>
      <dd></dd>
    </dl>
  </div>
...
<script>
import { getTopListsDetail } from "../../api/rege";
export default {
  data() {
    return {
      playlist:{
  ...
  methods: {
    getDetailFun() {
      ...
      getTopListsDetail({id:id}).then((data) => {
        console.log(data);
